...
public class HowManyDrinks {
	...
	public static String calculateDrinks(int age, int weight, String gender, String drinkType, int drinkNum, int hours) {
		...
		if (drinkType == "The Hard Stuff") {
			ouncesConsumed = (drinkNum * 1.5)*(0.40);
		}
		if (gender == "Male") {
		...
		output = String.format("%.2f", BAC);
		
		if(BAC < 0.08) {
			
			return "Yes you can drive. You are not drunk, your BAC is " + output;
		}
		else {
			return "No, you cannot drive. You are drunk, your BAC is " + output;
		}
